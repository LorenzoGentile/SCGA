...
#   create the first DOE                                                    ####
  ...
  bounds       <- sapply(feature[notdependent],getBounds )
  types        <- purrr::map(feature[notdependent],"type") %>% unlist

  independentLhd <- designLHD(lower = bounds[1,],upper = bounds[2,],control=list(size = size,types=types))
  ...
  counts   <- sort(counts,index.return=T)
  ...
  sapply(pop, checkCandidateLHD,feature,notdependent) %>% sum
  return(pop)
}
